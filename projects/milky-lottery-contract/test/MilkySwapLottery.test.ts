...
const MilkySwapLottery = artifacts.require("./MilkySwapLottery.sol");
...
contract("Lottery V2", ([alice, bob, carol, david, erin, operator, treasury, injector]) => {
  ...
  let _priceTicketInMilky = parseEther("0.5");
  ...
  // Contracts
  let lottery, mockMilky, randomNumberGenerator;
  ...
  before(async () => {
    // Deploy MockMilky
    mockMilky = await MockERC20.new("Mock MILKY", "MILKY", _totalInitSupply);

    // Deploy MockRandomNumberGenerator
    randomNumberGenerator = await MockRandomNumberGenerator.new({ from: alice });

    // Deploy MilkySwapLottery
    lottery = await MilkySwapLottery.new(mockMilky.address, randomNumberGenerator.address, { from: alice });
  ...
  describe("LOTTERY #1 - CUSTOM RANDOMNESS", async () => {
    ...
    it("Users mint and approve MILKY to be used in the lottery", async () => {
      for (let thisUser of [alice, bob, carol, david, erin, injector]) {
        await mockMilky.mintTokens(parseEther("100000"), { from: thisUser });
        await mockMilky.approve(lottery.address, parseEther("100000"), {
          from: thisUser,
        });
      }
    });
    ...
    it("Operator starts lottery", async () => {
      ...
      expectEvent(result, "LotteryOpen", {
        ...
        priceTicketInMilky: _priceTicketInMilky.toString(),
        firstTicketId: "0",
        injectedAmount: "0",
      });
    ...
    it("Bob buys 100 tickets", async () => {
      ...
      expectEvent.inTransaction(result.receipt.transactionHash, mockMilky, "Transfer", {
        from: bob,
        to: lottery.address,
    ...
    it("Owner does 10k MILKY injection", async () => {
    ...
    it("David claims the jackpot", async () => {
      // 10,000 + 47.525 + 0.5 + 4.9775 = 10053.0025 MILKY collected
      // 10053.0025 * (1-0.20) * 0.5 = 4021.201 MILKY
    ...
    it("Bob claims 10 winning tickets he bought", async () => {
      // 10,000 + 47.525 + 0.5 + 4.9775 = 10053.0025 MILKY collected
      // 10053.0025 * (1-0.20) * 0.02 = 160.84804 MILKY
      // 10053.0025 * (1-0.20) * 0.03 = 241.27206 MILKY
    ...
    describe("LOTTERY #2 - CUSTOM RANDOMNESS - Exceptions", async () => {
      ...
      it("Operator cannot start lottery if length is too short/long", async () => {
        ...
        await expectRevert(
          lottery.startLottery(endTimeTarget, _priceTicketInMilky, _discountDivisor, _rewardsBreakdown, _treasuryFee, {
            from: operator,
          }),
      ...
      it("Operator cannot start lottery if discount divisor is too low", async () => {
        ...
        await expectRevert(
          lottery.startLottery(endTime, _priceTicketInMilky, _discountDivisor, _rewardsBreakdown, _treasuryFee, {
            from: operator,
          }),
      ...
      it("Operator cannot start lottery if ticket price too low or too high", async () => {
        let newPriceTicketInMilky = parseEther("0.0049999999");

        await expectRevert(
          lottery.startLottery(endTime, newPriceTicketInMilky, _discountDivisor, _rewardsBreakdown, _treasuryFee, {
            from: operator,
          }),
          "Outside of limits"
        );

        newPriceTicketInMilky = parseEther("0.0049999999");
      ...
      it("Operator cannot start a second lottery", async () => {
        await expectRevert(
          lottery.startLottery(_lengthLottery, _priceTicketInMilky, _discountDivisor, _rewardsBreakdown, _treasuryFee, {
            from: operator,
          }),
    ...
    describe("Role exceptions", async () => {
      it("Owner can recover funds only if not MILKY token", async () => {
        // Deploy Random Token
        ...
        await expectRevert(
          lottery.recoverWrongTokens(mockMilky.address, parseEther("1"), { from: alice }),
          "Cannot be Milky token"
        );
      });
      ...
      it("Only owner/injector can call owner functions", async () => {
        ...
        await expectRevert(
          lottery.recoverWrongTokens(mockMilky.address, parseEther("10"), { from: operator }),
          "Ownable: caller is not the owner"
        );
